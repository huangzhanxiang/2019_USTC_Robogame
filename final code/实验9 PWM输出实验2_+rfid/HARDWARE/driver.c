...
void Locate_Rle(long num,u32 frequency,DIR_Type dir) //相对定位函数
{
	...
	motor_dir2=dir;//得到方向	
	DRIVER_DIR=motor_dir2;//设置方向
	
	if(motor_dir2==mCW)//顺时针
		target_pos=current_pos+num;//目标位置
	else if(motor_dir2==mCCW)//逆时针
		target_pos=current_pos-num;//目标位置
